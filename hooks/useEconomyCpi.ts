/**
 * Cached fetcher for `GET /api/economy/cpi` (CPIAUCSL index levels).
 */
...
function errorMessageFromCaught(e: unknown): string {
  if (e instanceof EconomyCpiApiError) {
    if (e.status === 503) {
...
export type UseEconomyCpiResult = {
  data: EconomyCpiResponse | null;
  isLoading: boolean;
  error: string | null;
  refetch: () => void;
};

export function useEconomyCpi(): UseEconomyCpiResult {
  const [data, setData] = useState<EconomyCpiResponse | null>(() => {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const res = await fetchEconomyCpi(ac.signal);
