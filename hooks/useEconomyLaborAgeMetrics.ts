/**
 * Cached fetcher for `GET /api/economy/labor/age-metrics`.
 */
import { useCallback, useEffect, useRef, useState } from "react";

import {
  fetchLaborAgeMetrics,
  LaborAgeMetricsApiError,
  type LaborAgeMetricsFetchParams,
  type LaborAgeMetricsResponse,
} from "@/hooks/api/economyLaborAgeMetricsApi";
...
type CacheEntry = {
  fetchedAt: number;
  data: LaborAgeMetricsResponse;
};

const cacheByWindow = new Map<string, CacheEntry>();

function windowCacheKey(params?: LaborAgeMetricsFetchParams): string {
...
export type UseEconomyLaborAgeMetricsResult = {
  data: LaborAgeMetricsResponse | null;
  isLoading: boolean;
  error: string | null;
  refetch: () => void;
};

export function useEconomyLaborAgeMetrics(
  params?: LaborAgeMetricsFetchParams,
): UseEconomyLaborAgeMetricsResult {
  const cacheKey = windowCacheKey(params);

  const [data, setData] = useState<LaborAgeMetricsResponse | null>(() => {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const res = await fetchLaborAgeMetrics(ac.signal, params);
