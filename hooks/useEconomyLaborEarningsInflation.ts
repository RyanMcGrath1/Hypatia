/**
 * Cached fetcher for `GET /api/economy/labor/earnings-inflation` (hourly earnings + CPI).
 */
import { useCallback, useEffect, useRef, useState } from "react";

import { EconomySectorApiError } from "@/hooks/api/economySectorApi";
import {
  fetchLaborEarningsInflation,
  type LaborEarningsInflationFetchParams,
  type LaborEarningsInflationResponse,
} from "@/hooks/api/economyLaborEarningsInflationApi";
...
type CacheEntry = {
  fetchedAt: number;
  data: LaborEarningsInflationResponse;
};

const cacheByWindow = new Map<string, CacheEntry>();

function windowCacheKey(params?: LaborEarningsInflationFetchParams): string {
...
export type UseEconomyLaborEarningsInflationResult = {
  data: LaborEarningsInflationResponse | null;
  isLoading: boolean;
  error: string | null;
  refetch: () => void;
};

export function useEconomyLaborEarningsInflation(
  params?: LaborEarningsInflationFetchParams,
): UseEconomyLaborEarningsInflationResult {
  const cacheKey = windowCacheKey(params);

  const [data, setData] = useState<LaborEarningsInflationResponse | null>(() => {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const res = await fetchLaborEarningsInflation(ac.signal, params);
